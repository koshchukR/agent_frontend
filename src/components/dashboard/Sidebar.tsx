      ...
      <div className="p-4 flex items-center justify-center h-16 border-b border-indigo-700">
        {isOpen ? (
          <div className="flex items-center">
            <img
              src="/shutterstock_2654438429.jpg"
              alt="Screen IQ Logo"
          ...
          <nav className="mt-2 px-2 space-y-1">
            {secondaryNavItems.map((item, index) => (
              <Link
                key={index}
                to={item.path}
                className={`flex items-center px-4 py-3 text-indigo-100 hover:bg-indigo-700 rounded-md transition-colors ${
                  item.active ? "bg-indigo-900 text-white" : ""
                } ${!isOpen && "justify-center"}`}
              >
                {item.icon}
                {isOpen && <span className="ml-3">{item.label}</span>}
